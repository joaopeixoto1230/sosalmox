...
import { patchStatusManual, estoqueAoEditar } from './contagem'
...
export default function MaterialCard({ material, evento, especie }) {
  ...
  async function trocarStatus(novoStatus) {
    ...
    setMenuAberto(false)
    try {
      // A regra do par status × estoque vive em `patchStatusManual` — a mesma
      // troca existe no card da Saída, e duas cópias divergiriam calado.
      ...
      <div className="space-y-0.5 text-xs text-gray-500 mb-3">
        ...
        {material.status === 'em_evento' && material.eventoAtual && (
          <p className={`font-medium truncate ${
            evento?.tipo === 'locacao_mensal' ? 'text-purple-600'
            : evento?.tipo === 'sublocacao' ? 'text-teal-600'
            : 'text-yellow-600'}`}>
            📍 {evento ? evento.nome : material.eventoAtual}
            {evento?.local ? ` · ${evento.local}` : ''}
            {evento?.tipo === 'locacao_mensal' ? ' (locação)' : ''}
            {evento?.tipo === 'sublocacao' ? ' (sublocação)' : ''}
          </p>
        )}
        {material.observacao && (
  ...
  async function salvar() {
    ...
    try {
      await updateDoc(doc(db, 'materiais', material.id), {
        ...
        estoqueAtual: estoqueAoEditar({ ...material, porQuantidade: form.porQuantidade === true }, form.status, form.estoqueAtual),
